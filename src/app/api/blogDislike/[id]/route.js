...
export async function PUT(request, content) {
  ...
  try {
    ...
    // Check if user's email is in the like array
    const likeIndex = blog.like.findIndex(
      (likeObj) => likeObj.userEmail === loggedInUserEmail
    );

    if (likeIndex !== -1) {
      // Remove user's email from the like array
      blog.like.splice(likeIndex, 1);
    }

    // Check if user's email is already in the dislike array
    const dislikeIndex = blog.dislike.findIndex(
      (dislikeObj) => dislikeObj.userEmail === loggedInUserEmail
    );

    if (dislikeIndex === -1) {
      // Add user's email to the dislike array
      blog.dislike.push({ userEmail: loggedInUserEmail });
    } else {
